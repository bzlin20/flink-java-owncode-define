...
/**
 * @ClassName: SideOutputProcessFunction
 * @Description:  随机进行side-output输出，仅供测试。
 * @author: ds-longju
 * @Date: 2022-09-04 09:24
 * @Version 1.0
 **/
public class SideOutputProcessFunction<T> extends ProcessFunction<T, T> {
    ...
    @Override
    public void processElement(T event, ProcessFunction<T, T>.Context ctx, Collector<T> out) throws Exception {
        out.collect(event);
